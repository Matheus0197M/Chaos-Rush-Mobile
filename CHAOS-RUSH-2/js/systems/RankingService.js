...
export const fetchRankings = async (limit = 10) => {
...
const getComparableRows = async ({ userId, email, nome }) => {
  const filters = [
...
const buildPayloadAttempts = ({ userId, nome, email, maxPontuacao, maxLevel }) => [
  {
    user_id: userId,
    nome,
    email,
...
export const saveBestRanking = async ({ pontuacao = 0, level = 1 } = {}) => {
  ...
  const { rows, matchColumn, matchValue } = await getComparableRows({ userId, email, nome });
  ...
  const attempts = buildPayloadAttempts({ userId, nome, email, maxPontuacao, maxLevel });
  const errors = [];

  for (const payload of attempts) {
    const query = rows.length && matchColumn
      ? supabase.from(RANKING_TABLE).update(payload).eq(matchColumn, matchValue)
